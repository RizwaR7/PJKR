        ...
        <div class="col-lg-12">
            <div class="card mb-4">
                <h4 class="card-header font-weight-bold text-white text-center"
                    style="background: linear-gradient(-90deg, #0d5959 0%, #016666 100%);">Kelola Pengumuman
                </h4>
                ...
                <div class="table-responsive p-3 text-center">
                    <table class="table table-bordered table-hover table-sm tb-menu-utama" id="dataTable" width="100%"
                        cellspacing="0">
                ...
                <div class="modal-body">
                    <form id="formCommon">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="foto_berita" name="foto_berita" placeholder="Cover"
                                required>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label for="ts">Tanggal Pengumuman</label>
                            <input type="date" class="form-control" id="ts" name="ts"
                                placeholder="Tanggal Pengumuman" required>
                        </div>
                        <div class="form-group">
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        var table = $("#dataTable").DataTable({
            ...
            serverSide: true,
            order: [1, "desc"],
            ajax: "{{ route('informasi_pengumuman.index') }}",
            columns: [{
                    data: "foto_berita",
                    name: "foto_berita",
                    render: function(data, type, row) {
                        // Render kolom gambar
                        return data ?`<img src="${data}" alt="Foto Pengumuman" width="150" height="100" style="object-fit: cover;" />`:
                ...
                },
                {
                    data: function(data) {
                        var baseUrl = "{{ url('/') }}";
                        var fullUrl = baseUrl + "/pengumuman/detail/" + data.id;
        ...
        $("#btnModal").click(function() {
            $("#saveBtn").val("create-pengumuman");
            $("#saveBtn").html("Tambah Pengumuman");
            $("#modalTitle").html("Tambah Pengumuman");
        ...
        $("body").on("click", ".edit", function() {
            var id = $(this).data("id");
            $.get(
                "{{ route('informasi_pengumuman.index') }}" + "/" + id + "/edit",
                function(data) {
                    $("#modalTitle").html("Edit Pengumuman");
                    $("#saveBtn").val("edit-Pengumuman");
                    $("#saveBtn").html("Edit Pengumuman");
                    $("#modalLabel").modal("show");
                    $("#id").val(data.id);
        ...
        $("#saveBtn").click(function(e) {
            ...
            $.ajax({
                ...
                url: "{{ route('informasi_pengumuman.store') }}",
                type: "POST",
                dataType: "json",
        ...
        $("body").on("click", ".delete", function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "DELETE",
                        url: "{{ route('informasi_pengumuman.store') }}" + "/" + id,
                        success: function(data) {
                            table.draw();
                        },
                        error: function(data) {
