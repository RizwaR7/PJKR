                ...
                <div class="col-md-8 col-lg-6">
                    <form action="{{ route('berita.index') }}" method="GET" class="d-flex" id="searchForm">
                        <input type="text" name="query" id="searchInput" class="form-control flex-grow-1"
                            placeholder="Cari Berita" value="{{ request('query') }}" style="width: auto;">
                    </form>
                </div>
            </div>
        </div>
            ...
            <div class="col-lg-8">
                ...
                <div class="featured-post mb-4 card">
                    <a href="{{ route('detail_berita.index', ['id' =>  $berita->id]) }}" class="newsdesk" style="color: inherit;">
                        @if (is_null($berita->foto_berita))
                        @elseif ($berita->foto_berita)
                        <img src="{{ $berita->foto_berita }}" class="img-fluid card-img-top banner-img" style="display: block;object-fit: cover;object-position: center;height: 100%;max-height: 200px;width: 100%;">
                        @endif
                        <p class="h5">{{ $berita->judul }}</p>
                        <p>{{ \Carbon\Carbon::createFromTimestamp($berita->ts)->format('F d, Y') }}
                            <span>
                                <span
                                    class="badge md-badge badge-{{ $berita->jenis == 'Berita' ? 'primary' : ($berita->jenis == 'Kegiatan' ? 'secondary' : ($berita->jenis == 'Pengumuman' ? 'success' : 'danger')) }} text-white">{{ $berita->jenis }}</span>
                                <span class="badge md-badge ml-2 badge-info text-white">{{ $berita->counters }}
                                    Dilihat</span>
                            </span>
                        </p>
